...
int main(int argc, char *argv[]) {
  ...
  while (counter < 5) {
    ...
    if (establishedConnectionFD < 0) {
      fprintf(stderr, "otp_dec_d error: Cannot accept connection.\n");
    }
    ...
    case 0:
      ...
      // Checks if the message came from otp_dec
      ...
      // Obtaining the key and ciphertext using tokens
      token = strtok(buffer, "@");
      strcpy(ciphertextBuff, token);
      token = strtok(NULL, "@");
      ...
      // Variables used as part of decryption process
      char charPT;
      char *charKey;
      char *charCT;
      int indexPT;
      ...
      // Clearing out the plaintext buffer
      memset(plaintext, '\0', sizeof(plaintext));

      // Loops through and applies the decryption algorithm to each character
      // of ciphertext
      for (int i = 0; i < strlen(ciphertextBuff); i++) {
        charCT = strchr(charSet, ciphertextBuff[i]);
        indexCT = (int) (charCT - charSet);

        charKey = strchr(charSet, keyBuff[i]);
        indexKey = (int) (charKey - charSet);

        indexPT = (indexCT - indexKey) % 27;
        if (indexPT < 0) {
          indexPT = indexPT + 27;
        }
        charPT = charSet[indexPT];
        plaintext[strlen(plaintext)] = charPT;
      }
      // Adds a trailing "\n" character to the plaintext buffer
      plaintext[strlen(plaintext)] = '\n';

      // Sends the decrypted ciphertext (plaintext) back to the client
      charsWritten =
